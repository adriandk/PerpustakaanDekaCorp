<?php
    ...
    $sql = "SELECT transaksi.idTransaksi, siswa.nama, siswa.nis, transaksi.tglPinjam, siswa.tingkat, siswa.jurusan, siswa.kelas, detailtransaksi.tglKembali FROM transaksi, siswa, detailtransaksi WHERE transaksi.nis = siswa.nis AND transaksi.idTransaksi = detailtransaksi.idTransaksi AND detailtransaksi.status = 1 GROUP BY idTransaksi";
?>
<!-- ini belum -->
<!DOCTYPE html>
<html class="h-100">
                ...
                <tr class="thead-dark">
                    ...
                    if (isset($_POST['searchbtn'])) {
                        ...
                        if (empty($search)) {
                            ...
                            $sql = "SELECT transaksi.idTransaksi, siswa.nama, siswa.nis, transaksi.tglPinjam, siswa.tingkat, siswa.jurusan, siswa.kelas, detailtransaksi.tglKembali FROM transaksi, siswa, detailtransaksi WHERE transaksi.nis = siswa.nis AND transaksi.idTransaksi = detailtransaksi.idTransaksi AND detailtransaksi.status = 1 AND siswa.nama like '%$search%' GROUP BY idTransaksi";
                        }
                    }
                    ...
                    while ($row = $query->fetch(PDO::FETCH_ASSOC)) {
                        ...
                        echo "<td scope='col' style='text-align: center;'>".$row['tglKembali']."</td>";
                        echo "<td scope='col' style='text-align: center;'>
                        <a href =\"kembalikanbuku.php?id=$row[idTransaksi]&kelas=$kelas&stat=laporan\"><button type='submit' class='btn btn-primary'>Detail</button> &nbsp;
                        <a href =\"deletetransaksi.php?id=$row[idTransaksi]\" onClick =\"return confirm ('Are You Sure?')\"><img src='assets/images/hapus.png' style='width: 30px; height: 30px;'></a></td>";
                    }
